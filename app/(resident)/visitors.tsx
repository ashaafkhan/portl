...
import { Users, Ticket, Plus } from 'lucide-react-native';
...
export default function VisitorsScreen() {
  ...
  const [activeTab, setActiveTab] = useState<'passes' | 'create'>('passes');
    ...
    return (
      <View className="bg-white p-5 rounded-2xl border border-neutral-200 mb-4 flex-row items-center justify-between shadow-sm">
        ...
        <View className={`px-4 py-2 rounded-lg items-center justify-center ${isActive ? 'bg-indigo-50 border border-indigo-200' : 'bg-neutral-100'}`}>
          <Text className={`font-mono text-xl font-bold tracking-widest ${isActive ? 'text-indigo-600' : 'text-neutral-400 line-through'}`}>
            {item.qr_token}
          </Text>
        </View>
      </View>
  ...
  return (
    <SafeAreaView className="flex-1 bg-neutral-50" edges={['top']}>
      <View className="px-6 py-4 bg-white border-b border-neutral-200">
        <Text className="text-2xl font-bold text-neutral-900">Pre-approvals</Text>
      </View>

      <View className="flex-row p-4 gap-2">
        <TouchableOpacity 
          onPress={() => setActiveTab('passes')}
          className={`px-4 py-2 rounded-full ${activeTab === 'passes' ? 'bg-[#FF7A59]' : 'bg-neutral-200'}`}
        >
          <Text className={`font-semibold ${activeTab === 'passes' ? 'text-white' : 'text-neutral-600'}`}>My Passes</Text>
        </TouchableOpacity>
        <TouchableOpacity 
          onPress={() => setActiveTab('create')}
          className={`px-4 py-2 rounded-full ${activeTab === 'create' ? 'bg-[#FF7A59]' : 'bg-neutral-200'}`}
        >
        ...
        <ScrollView className="flex-1 p-6">
          <View className="bg-white p-6 rounded-2xl border border-neutral-200 shadow-sm">
            ...
            <TouchableOpacity 
              ...
              disabled={submitting}
              className={`p-4 rounded-xl flex-row justify-center items-center gap-2 ${submitting ? 'bg-coral-400' : 'bg-coral-500'}`}
              style={{ backgroundColor: submitting ? '#FFA38C' : '#FF7A59' }}
            >
              {submitting ? (
                ...
                <>
                  ...
                  <Text className="text-white font-bold text-lg">Generate Pass</Text>
                </>
              )}
            </TouchableOpacity>
          </View>
        ...
        <View className="flex-1 px-6 pt-4">
          {loading ? (
            ...
            <EmptyState 
              icon={Ticket}
              title="No Passes"
              description="You haven't generated any guest passes."
            />
          ) : (
            <FlatList
              data={passes}
